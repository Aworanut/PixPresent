...
- [ ] เพิ่ม column: `tenants.phone TEXT null` — เบอร์โทรสำหรับติดต่อ/การตลาด (optional)
...
- [ ] เขียน config `lib/credit-packages.ts` (tier limits + costs)
- [ ] เพิ่ม migration: `events.tier`, `events.storage_limit_gb`, `events.link_active_days`, `events.data_retention_days`
- [ ] Welcome bonus trigger ใน signup flow (#4)
...
**Acceptance criteria**
- [ ] `lib/payment-config.ts` — bank info, top-up packages, QR image path
- [ ] Slip upload endpoint รับ multipart + เก็บ slip ใน R2 path `/slips/{tenant_id}/{slip_id}.jpg`
- [ ] เรียก SlipOK API verify slip → ถ้าผ่าน auto-approve + credit ledger insert (atomic)
- [ ] Fallback: ถ้า SlipOK ไม่ available → slip status = pending รอ manual admin (ใน #19)
- [ ] Atomic transaction approve + credit balance update (Postgres RPC)
- [ ] Credit ledger immutable (append-only — ไม่ allow update/delete ผ่าน RLS)
- [ ] API ดึง credit balance ปัจจุบัน + ledger history
- [ ] Email admin (woranut.ak@gmail.com) เมื่อมี slip pending (fallback กรณี SlipOK fail)
- [ ] Email organizer เมื่อ top-up approved / rejected (ใช้ Resend หรือ Supabase email)
- [ ] Unit tests: double top-up, SlipOK verify fail, credit ไม่พอตอนสร้าง event
- [ ] Integration test: signup → topup → SlipOK verify → credit เพิ่ม → สร้าง event
...
**Acceptance criteria**
- [ ] Credit indicator แสดงทุก authenticated page ✅ (ทำแล้วใน #12)
- [ ] Top-up: preset + custom + QR + bank info + slip upload
- [ ] Credit history แสดง ledger + slip status
- [ ] หักเครดิตตอนสร้าง event (atomic: credit deduct + event insert)
- [ ] ถ้า credit ไม่พอ → block create + redirect top-up
- [ ] Delete ก่อน Import → auto-refund + delete event (atomic)
- [ ] Delete หลัง Import → ปุ่มลบถูก disable หรือ confirm "ไม่คืน credit"
