...
- [ ] Integration test: signup → topup → SlipOK verify → credit เพิ่ม → สร้าง event (รอ real creds)
...
**Acceptance criteria**
- [ ] Cron route `app/api/cron/cleanup-collections/route.ts` (Vercel cron) หรือ pg_cron ใน Supabase
...
- [ ] Guard ใน share link: ถ้า guest เข้า link ที่ retention หมดแล้ว → แสดง "ข้อมูลงานนี้หมดอายุแล้ว"
- [ ] Alert: ถ้า DeleteCollection fail → log error + email admin (ป้องกัน orphaned collections ที่เสีย AWS cost)
