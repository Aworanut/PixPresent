...
**Type:** AFK
**Related:** #7, #20
**Status:** partially shipped 2026-06-03 (commits `e72d37e`, `8e90801`, `c1fcf92`) — **prod verify + tuning ค้าง** (paused เพื่อทำ main feature)
...
**ค้าง — กลับมาทำต่อ**
- [ ] **Verify prod:** sync event รูปเยอะ → auto-resume เดินข้าม timeout จนจบ (toast ไม่ค้าง); UI lint changes (gallery tab/filter, lightbox spinner, drive modal, avatar) ยังทำงาน
- [ ] อ่าน `[sync-perf]` baseline จาก Vercel logs → ดูว่า step ไหนครองเวลา
- [ ] **#3** ลด full variant (ปัจจุบัน 8000px + mozjpeg q95 — น่าจะเป็นตัวกิน `process` เยอะสุด) — ทำถ้า baseline ชี้ว่า process ครองเวลา
- [ ] **#5** เพิ่ม retry ให้ Drive download (`google-drive-api.ts` เรียกตรงไม่มี backoff; Dropbox มี `withDropboxRetry` แล้ว)
- [ ] **#6** ใช้ SQL SUM แทนรวม `storage_bytes` ใน JS (scale ไม่ดีเมื่อรูปหลักหมื่น)
