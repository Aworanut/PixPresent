...
- [ ] รองรับ 1,000 รูปต่อ event โดยไม่ timeout (NFR §8.1) — ⚠️ เคลม "verified" เดิม **ไม่จริงบน Vercel Hobby** (60s function cap → timeout กลางคัน). Addressed ใน **#21** (concurrency + client auto-resume, shipped 2026-06-03); รอ re-verify บน prod
...
- [ ] **Verify ก่อน pilot:** full-sync ~500 รูปบน prod ไม่ชน serverless timeout (หรือ resume ได้) — re-verify เคลม #7 "1,000 รูป" → **addressed ใน #21** (concurrency + auto-resume shipped 2026-06-03); รอ verify prod

**Blocked by:** None (seam พร้อม) — ต้องสมัคร Dropbox app (HITL) ก่อนทดสอบ e2e

---

### #21 — Sync Performance Hardening (Hobby 60s timeout fix)

**Type:** AFK
**Related:** #7, #20
**Status:** partially shipped 2026-06-03 + 2026-06-04 (full-variant #3 + Drive retry #5 + baseline อ่านแล้ว) — **prod-verify + SQL SUM #6 ค้าง**

**ปัญหา**

Sync ภาพช้าและ "ค้าง" บน prod. Diagnose เจอ 2 ตัวการ: (1) process รูปแบบ serial ทีละใบ — แต่ละใบเป็น network wait (download → R2 → Rekognition) ที่ไม่ overlap; (2) ไม่มี `maxDuration`/resume → Vercel Hobby ตัด function ที่ 60s กลางคัน แล้ว client toast ค้าง (เคลม #7 "1,000 รูป verified" จึงไม่จริงบน Hobby).

**Shipped (2026-06-03)**
- [x] Concurrency worker pool (`SYNC_CONCURRENCY`=5, sliding) แทน serial loop + reuse Rekognition client ข้ามรูป → ~3-5x (user-confirmed "เร็วขึ้นมาก")
- [x] `[sync-perf]` per-step timing logs (download/process/upload/rekognition/insert) ใน Vercel runtime logs
- [x] `export const maxDuration = 60` (Hobby cap)
- [x] Client auto-resume (`_event-toolbar.tsx`): re-POST `/sync` จน terminal event, skip รูปเดิมด้วย doneSet + guard กันลูปไม่จบ
- [x] Client handle `storage_exceeded` event (เดิม ignore → toast ค้าง)

**Shipped (2026-06-04)**
- [x] อ่าน `[sync-perf]` baseline (prod): `process` ครองเวลา **62%** — avg/photo process=9783ms rekognition=2768ms upload=2451ms download=578ms insert=270ms (6 รูป/window แล้ว timeout)
- [x] **#3** ลด full variant: `8000px/q95/mozjpeg` → `4000px/q90/libjpeg` (`lib/image-processing.ts`) + regression test. Local bench (noise=worst case) 14.4s→0.32s; รูปจริงคาด process ~9.8s→~1-1.5s, upload ลด (ไฟล์เล็ก), R2 full storage ลด ~3-5x
- [x] **#5** Drive retry/backoff: `withDriveRetry` ห่อ `listImages`+`downloadFile` (429/5xx/403-ratelimit/transient-net, exp backoff) mirror Dropbox + unit test
- [x] พบ `process` เป็น **CPU-bound** ไม่ใช่ network (comment `route.ts` เดิมเดาผิด) → เพิ่ม `SYNC_CONCURRENCY` ไม่ช่วย stage นี้

**ค้าง — กลับมาทำต่อ**
- [ ] **Verify prod (#3 prod-verify):** re-sync event รูปเยอะหลัง deploy 2026-06-04 → อ่าน `[sync-perf]` ใหม่ยืนยัน process ลดจริง + auto-resume เดินจนจบ (toast ไม่ค้าง); UI lint changes ยังทำงาน
- [ ] **#6** ใช้ SQL SUM แทนรวม `storage_bytes` ใน JS (scale ไม่ดีเมื่อรูปหลักหมื่น) — ต้อง migration (RPC)
- [ ] tune `SYNC_CONCURRENCY` หลัง process เล็กลง — re-measure ก่อน (I/O-bound มากขึ้น อาจเพิ่มได้)
- [ ] อัพเดท #7 acceptance "1,000 รูป" ให้สะท้อนความจริงหลัง verify prod

**หมายเหตุระยะยาว:** maxDuration อย่างเดียวไม่พอถ้ารูปเป็นพันๆ — auto-resume ช่วยได้ แต่ทางที่ถูกระยะยาวคือ background job/queue (เกิน scope ตอนนี้).

**Blocked by:** None — แต่ tuning #3 รอ `[sync-perf]` baseline ก่อน
...
**Status:** 🟢 core done (local) 2026-06-11 — tsc+lint+build เขียว, ทุก route ขึ้นใน build. **ค้าง (polish):** ref-faces panel + เพิ่ม ref face จาก detail (upload/tag) — actions พร้อมแล้ว. คลิกจริงยังไม่ได้ verify

**Acceptance criteria**

- [x] `lib/people/queries.ts`: `listPeople`, `getPersonPhotos`, `getPendingMatches`, `getTenantEvents`, `getPendingScanCount` (user-client → RLS)
- [x] `app/dashboard/people/page.tsx`: list + search (`?q=`) + scan-runner banner + business gate
- [x] `app/dashboard/people/[id]/page.tsx`: photo grid (`status='confirmed'`) + filter by event. *(ref-faces panel → polish)*
- [x] Pending review panel: `status='pending'` → confirm/reject (Server Action inline forms)
- [ ] เพิ่ม ref face จาก person detail (upload/tag) — **polish** (action `addUploadedRefFaceAction`/`addTaggedRefFaceAction` พร้อม)
- [x] ลบคน (confirm dialog → cascade) — `DeletePersonButton` + `deletePersonAction`
- [x] Nav link "บุคคล" ใน dashboard header (business tier เท่านั้น)
- [x] Client scan runner (auto-resume, `_scan-runner.tsx`) — ย้ายจาก #25
...
**Status:** 🟢 done (local) 2026-06-11 — `enqueueEventScans` wired after sync `done`. tsc+lint+build เขียว

**Acceptance criteria**

- [x] ใน `app/api/events/[id]/sync/route.ts`: หลัง sync `done` → `enqueueEventScans` upsert `person_event_scans` (ทุก person × event นั้น) = `pending` (best-effort, ไม่ block response)
- [x] ถ้า tenant ไม่มี people → skip (no-op ใน `enqueueEventScans`)
- [~] execution: pending units รันผ่าน scan-runner บนหน้า `/dashboard/people` (#26). *(auto-fire จาก sync toolbar ทันที = polish ภายหลัง)*
- [ ] test: sync event ที่มี people → pending rows เพิ่ม — **ยังไม่เขียน** (logic ตรงไปตรงมา, ครอบด้วย build)
...
**Status:** 🟢 done (local) 2026-06-11 — ทำสำเร็จเป็น byproduct ของ #24/#26. tsc+lint+build เขียว

**Acceptance criteria**

- [x] Business-tier check ทุก Server Action (`requireBusinessTenant` throw ถ้าไม่ใช่ business) + gate หน้า `/dashboard/people`
- [x] ลบคน: `deletePersonAction` ลบ R2 keys ทุกใบก่อน → delete person → DB cascade
- [x] Log `searchFacesByImage` calls/unit (`[people-scan] ... rekognition_calls=N` ใน matching.ts)
- [x] `/dashboard/people/[id]` แสดง PDPA note
- [x] Upgrade prompt: หน้า `/dashboard/people` แสดงข้อความ + ลิงก์ /account เมื่อไม่ใช่ business
